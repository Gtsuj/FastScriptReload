...
{
    /// <summary>
    /// 编译辅助类，提供使用 Roslyn 直接编译 C# 文件和生成热重载Hook程序集的功能
    /// </summary>
    public static class CompileHelper
    {
        ...
        public static bool CompileCsFiles(List<string> csFilePaths)
        {
            try
            {
                ...
                filesToCompile.UnionWith(ReloadHelper.HookTypeInfoCache.Values
                ...
                {
                    var syntaxTree = HotReloadCache.GetOrParseSyntaxTree(csFilePath);
                    if (syntaxTree == null)
                    {
                ...
                var references = HotReloadCache.GetOrResolveAssemblyReferences();
                ...
                if (emitResult.Success)
                {
                    ...
                    HotReloadCache.AssemblyStream = ms;

                    // var filePath = Path.Combine(ReloadHelper.AssemblyPath, assemblyName + ".dll");
        ...
        {
            var templateAssembly = HotReloadCache.AssemblyDefinition;
            ...
            foreach (var typeDef in mainModule.Types.ToArray())
            {
                ...
                {
                    if (info.ModifiedMethods.ContainsKey(methodDef.FullDescription()))
                    {
                    ...
                    else if (info.AddedMethods.ContainsKey(methodDef.FullDescription()))
                    {
                        var newMethodDef = ModifyMethod(mainModule, methodDef,
                            mainModule.ImportReference(info.ExistingType));
                        typeDef.Methods.Add(newMethodDef);
            ...
            // 生成文件名并保存
            var filePath = Path.Combine(ReloadHelper.AssemblyPath,
                $"{HotReloadCache.AssemblyDefinition.Name.Name}.dll");
        ...
        private static MethodDefinition ModifyMethod(ModuleDefinition module, MethodDefinition methodDef,
            TypeReference originalTypeRef)
        {
            // MethodDefinition newMethodDef = methodDef;
            ...
            newMethodDef.DeclaringType = methodDef.DeclaringType;
            ...
            // newMethodDef.Attributes = MethodAttributes.Public | MethodAttributes.Static;

            // 添加原方法参数
            foreach (var param in methodDef.Parameters)
            {
                newMethodDef.Parameters.Add(new ParameterDefinition(param.Name, param.Attributes,
            ...
            {
                processor.Append(CreateInstruction(module, sourceInstructions[i], genericParamMap));
            }
            ...
            Dictionary<GenericParameter, GenericParameter> genericParamMap = null)
        {
            ...
            {
                Instruction.Create(sourceInst.OpCode);
            }
            ...
            {
                // 处理新增方法调用
                if (ReloadHelper.HookTypeInfoCache.TryGetValue(methodRef.DeclaringType.FullName, out var hookType)
        ...
        private static void AddRequiredReferences(AssemblyDefinition wrapperAssembly, List<HookTypeInfo> hookTypeInfos)
        {
            ...
            foreach (var originalAssembly in originalAssemblies)
            {
                try
                {
                    ...
                    var originalAssemblyDef = HotReloadCache.GetOrReadAssemblyDefinition(originalAssembly.Location);
        ...
        private static MethodReference GetOriginalMethodReference(ModuleDefinition module, MethodReference methodRef)
        {
            try
            {
                ...
                var originalAssemblyDef = HotReloadCache.GetOrReadAssemblyDefinition(originalType.Assembly.Location);
                var originalTypeDef =
